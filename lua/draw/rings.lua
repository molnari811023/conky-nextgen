...
--]]
-- draw/rings.lua — Ring gauges: segmented or smooth arc mode
local RING_DEFAULT = {
	view = nil,
	group = nil,
	click = nil,
...
local function draw_ring_mode(cr, s, dv, ov)
	...
	for i = 1, s.sectors do
		...
		move_to_arc_start(cr, s.x, s.y, s.radius - s.thickness / 2, angle1)
...
local function draw_smooth_mode(cr, s, pct, ov)
	...
	cairo_set_source_rgba(cr, r, g, b, a)
	cairo_set_line_width(cr, s.thickness)
	move_to_arc_start(cr, s.x, s.y, s.radius - s.thickness / 2, start)
...
function draw_one_ring(cr, s0)
	...
	if not draw_allowed(s0.view, s0.group) then
		return
	end
	local s = {}
