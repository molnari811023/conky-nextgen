...
> Weblate-compatible, 16 languages ready (hu, en, de, fr, nl, es, pt, it, pl, tr, ja, zh_CN, ru, ro, hr, ar), `.pot` template for adding more.  
...
| # | Module | Purpose |
|---|--------|---------|
| 1 | translate | Weather UI translations via `.mo` files. 16 languages ready — see i18n section below |
...
- **16 languages shipped**: Hungarian (`hu`), English (`en`), German (`de`), French (`fr`), Dutch (`nl`), Spanish (`es`), Portuguese (`pt`), Italian (`it`), Polish (`pl`), Turkish (`tr`), Japanese (`ja`), Chinese (`zh_CN`), Russian (`ru`), Romanian (`ro`), Croatian (`hr`), Arabic (`ar`)
