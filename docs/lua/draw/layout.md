...
## How it works
Define a `layout` table (in `lua/widget.lua`). Each entry has a `name` and `height`. The engine computes `y_start_<name>` and `height_<name>` global variables, and `y_end_dynamic` for total height tracking.
...
## Padding
Default padding between sections: 7px (set via `PADDING` constant in `lua/draw/layout.lua`).
